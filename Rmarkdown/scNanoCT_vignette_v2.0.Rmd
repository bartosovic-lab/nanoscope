...
**Please note how in this tutorial we are describing how to perform the downstream analysis by using peaks, not bins. In addition, note how the biological data here analysed are 2 biological replicates of the same sample, thus it is safe to merge the two samples without any kind of integration. Often experiments are designed in order to have multiple samples and/or biological conditions requiring data integration, differential analysis, etc. These are not part of this vignette, but will be implemented in other vignettes in the future. **\
...
#### Astrocytes non-telencephalon - AST-NT (cluster 0)
...
#### Astrocytes telencephalon - AST-TE (clusters 2, 7)
```{r fig.width = 12, fig.height = 3}
FeaturePlot(
  object = combined.obj.ls$H3K27ac_ATAGAGGC,
...
#### Excitatory neurons - EXC (clusters 4, 15)
...
#### Inhibitory neurons - INH (clusters 6, 14)
```{r fig.width = 8, fig.height = 3}
FeaturePlot(
  object = combined.obj.ls$H3K27ac_ATAGAGGC,
...
#### Vascular smooth muscle cells - VSMC (cluster 9)
```{r fig.width = 12, fig.height = 3}
FeaturePlot(
  object = combined.obj.ls$H3K27ac_ATAGAGGC,
...
#### Olfactory ensheating cells - OEC (cluster 10)
```{r fig.width = 12, fig.height = 3}
FeaturePlot(
  object = combined.obj.ls$H3K27ac_ATAGAGGC,
...
#### Vascular leptomeningeal cells - VLMC (cluster 11)
```{r fig.width = 8, fig.height = 3}
FeaturePlot(
  object = combined.obj.ls$H3K27ac_ATAGAGGC,
...
```{r fig.width = 10, fig.height = 4}
# Plot each modality separately
p1=DimPlot(combined.obj.ls$ATAC_TATAGCCT, label = TRUE,repel = T, group.by = "layer2_annotation") + NoLegend() +
  ggtitle("ATAC") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))

p2=DimPlot(combined.obj.ls$H3K27ac_ATAGAGGC, label = TRUE,repel = T, group.by = "layer2_annotation") + NoLegend() +
  ggtitle("H3K27ac") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))

p3=DimPlot(combined.obj.ls$H3K27me3_CCTATCCT, label = TRUE,repel = T, group.by = "layer2_annotation") + NoLegend() +
  ggtitle("H3K27me3") +
  theme(plot.title = element_text(size=15,hjust=0.5,face='bold'))
